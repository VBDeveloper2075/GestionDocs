  ...
  const generatePDF = async () => {
    ...
    try {
      for (const nombre of nombres) {
        ...
        const contenido = `En Caseros, a los ....... días del mes de ${formData.fechaActa} de 2025, siendo las ........., se presenta en Secretaría de Asuntos Docentes de Tres de Febrero, sita en A. Ferreyra 2631 Caseros, el/la docente ${data.nombre_apellido} DNI ${data.dni}, para notificarse sobre ${data.expediente}, y su ${data.resolucion}, donde ${data.texto_adicional} en el distrito de Tres de Febrero.`;

        const notificacion = "El/la docente firma en autos y se envía al correo electrónico oficial declarado -@abc.gob.ar-, copia de la Resolución.";

        const splitContent = doc.splitTextToSize(contenido, 170);
        doc.text(splitContent, 20, 50);

        const splitNotification = doc.splitTextToSize(notificacion, 170);
        doc.text(splitNotification, 20, 90);

        doc.text("FIRMA:", 20, 120);
        doc.text("ACLARACIÓN:", 20, 135);
        doc.text("DNI:", 20, 150);
        doc.text("LUGAR Y FECHA:", 20, 165);
        doc.text("DOMICILIO CONSTITUIDO:", 20, 180);
        doc.text("CELULAR:", 20, 195);
        doc.text("CORREO ABC:", 20, 210);

        const nombreArchivo = `acta-notificacion-${data.nombre_apellido.toLowerCase().replace(/\s+/g, "-")}.pdf`;
      ...
      <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-lg p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <Label htmlFor="nombreApellido">Nombre y Apellido del docente (separar múltiples con punto y coma)</Label>
            <Input
              id="nombreApellido"
              name="nombreApellido"
